...
RFLINK_AURIOL_V3_Parse($$)
{
  ...
  my $time = time();
  if ($time_old ==0) {
  	Log 5, "RFLINK_AURIOL_V3: decoding delay=0 msg=$msg";
  } else {
  	my $time_diff = $time - $time_old ;
  	Log 5, "RFLINK_AURIOL_V3: decoding delay=$time_diff msg=$msg";
  }
  $time_old = $time;
  ...
  Debug("RFLINK_AURIOL_V3: call parse_RFLINK_AURIOL_V3_msg");
  my ($sensorname,$id,$tmp,$hum,$bat,$chn) = parse_RFLINK_AURIOL_V3_msg($msg);

  Debug("RFLINK_AURIOL_V3: parse_RFLINK_AURIOL_V3_msg=$sensorname,$id,$tmp,$hum,$bat,$chn");
  ...
  Debug("RFLINK_AURIOL_V3 parse, get longids setting...");
...
#  Debug("RFLINK parse: test if defined with $name");
  ...
  if ($hash){
    Debug ("RFLINK_AURIOL_V3: hash is: $hash");
  }else{
    Debug ("RFLINK_AURIOL_V3: hash is undef");
  }
  if($hash){
    ...
    Debug("RFLINK_AURIOL_V3 - return: UNDEFINED $sensorname RFLINK_AURIOL_V3 $sensorname");
